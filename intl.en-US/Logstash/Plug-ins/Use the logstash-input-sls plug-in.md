---
keyword: [logstash-input-sls plug-in, Logstash log retrieval from Log Service]
---

# Use the logstash-input-sls plug-in

logstash-input-sls is a built-in plug-in of Alibaba Cloud Logstash. This plug-in retrieves logs from Log Service.
...
-   Automatic load balancing: Shards are automatically allocated based on the number of consumers in a consumer group. If consumers join or leave the consumer group, logstash-input-sls automatically rebalances the shards.
...
-   A Log Service project and a Logstore are created. Data is collected.
    ...
    For more information, see [Get started with Log Service](/intl.en-US/Quick Start/Quick start.md).


## logstash-input-sls application

After the prerequisites are met, you can create a pipeline task by following the instructions provided in [Use configuration files to manage pipelines](). For more information about the prerequisites, see [Prerequisites](#section_gsb_1ka_93k). When you create a pipeline task, set the pipeline parameters by following the descriptions that are provided in the table of the Parameters section. After you set the parameters, you must save the settings and deploy the pipeline. Then, Alibaba Cloud Logstash is triggered to retrieve logs from Log Service.

The following example shows how to use Alibaba Cloud Logstash to consume log data in a Logstore and transfer the logs to Alibaba Cloud Elasticsearch.
...
Make the following assumptions: The Logstore has 10 shards. The data traffic of each shard is 1 MB/s. The processing capacity of each Alibaba Cloud Logstash server is 3 MB/s. Five Alibaba Cloud Logstash servers are allocated. Each server uses the same settings of `consumer_group` and `consumer_name`. `consumer_name_with_ip` is set to `true`.

In this case, each server is allocated two shards, and each processes data at 2 MB/s.
...
|---------|---------|---------|-----------|
|`endpoint`|String|Yes|The endpoint of the Log Service project in a VPC. For more information, see [Endpoints for the classic network and VPC](/intl.en-US/Developer Guide/API Reference/Endpoints.md).|
...
|`consumer_name`|String|Yes|The name of a consumer, which can be customized. The names of consumers must be unique in a consumer group. Otherwise, undefined behavior may occur.|
|`position`|String|Yes|The consumer position. Valid values: -   `begin`: consumes data from the first log entry that is written to the Logstore.
-   `end`: consumes data from the current point in time.
-   `yyyy-MM-dd HH:mm:ss`: consumes data from a specified point in time. |
|`checkpoint_second`|Number|No|The interval for triggering a checkpoint. We recommend that you set the interval to a value in the range from 10s to 60s. Minimum value: 10s. Default value: 30s.|
...
-   Test environment
    ...
    -   Memory: 8 GB
    -   OS: Linux
-   Configuration of Alibaba Cloud Logstash
...
-   Test procedure
    ...
    2.  Start Alibaba Cloud Logstash to consume data in the Logstore. Make sure that the consumption latency does not increase and the consumption speed can keep pace with the production speed.
-   Test result
