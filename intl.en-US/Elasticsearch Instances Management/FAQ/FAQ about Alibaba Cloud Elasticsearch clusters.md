...
-   FAQ about the purchase, subscription cancellation, or release of clusters
    -   [When I purchase an Elasticsearch cluster, I selected an incorrect zone. How do I change the zone?](#section_ser_ndn_afb)
    ...
    -   [How do I access an Elasticsearch cluster from the classic network?](#buya)
    -   [How do I release an Elasticsearch cluster?](#section_p92_cav_ju9)
    -   [When is an Elasticsearch cluster released after it is suspended?](#buyb)
...
-   FAQ about features
    ...
    -   [Does Elasticsearch support LDAP?](#section_0b9_ew5_2bn)
...
-   FAQ about cluster configuration and configuration updates
    ...
    -   [In the event of a temporary business surge, how do I modify the configuration of an Elasticsearch cluster to ensure that services run as expected?](#section_rdh_36n_v9k)
    ...
    -   [Can I use the YML configuration file of an Elasticsearch cluster to change the settings of the http.max\_content\_length and discovery.zen.ping\_timeout parameters?](#section_e54_c8p_2ei)
    -   [Can I switch the VPC of an Elasticsearch cluster?](#section_sse_1yg_dng)
-   FAQ about plug-ins, tokens, and synonyms
    ...
    -   [After I update IK dictionaries, how do I apply the new dictionaries to existing data?](#section_ewc_qn2_fey)
    -   [Is a threshold specified for full GC?](#section_4k3_xz1_0o2)
    -   [Can I remove plug-ins that are not used?](#ra)
    -   [Are the dictionaries provided by the IK analysis plug-in of Alibaba Cloud Elasticsearch the same as those provided by the IK analysis plug-in of open source Elasticsearch?](#rb)
...
-   FAQ about logs
    ...
    -   [I can view Elasticsearch cluster logs that are generated only over the last seven days. How do I view more logs?](#section_6qv_ism_v2t)
...
-   FAQ about data backup and restoration
    ...
    -   [I enable the Auto Snapshot feature but do not specify shared OSS repositories for an Elasticsearch cluster. Are snapshots created?](#section_pj5_ckd_rdv)
...
-   FAQ about access to clusters
    ...
    -   [I purchase an ECS instance that resides in the same VPC as but different zone from an Elasticsearch cluster. Can I use the ECS instance to access the Elasticsearch cluster from an internal network?](#visita)
    -   [How do I access an Elasticsearch cluster from the Internet?](#visitb)

## When I purchase an Elasticsearch cluster, I selected an incorrect zone. How do I change the zone?

After the cluster is created and in the Active state, migrate nodes from the incorrect zone to your desired zone. For more information, see [Migrate nodes in a zone](/intl.en-US/Elasticsearch Instances Management/Data migration/Migrate nodes in a zone.md).
...
You can use the ClassicLink feature to access an Elasticsearch cluster from the classic network. For more information, see [Access to an Alibaba Cloud Elasticsearch cluster from the classic network](/intl.en-US/Elasticsearch Instances Management/FAQ/Access to an Alibaba Cloud Elasticsearch cluster from the classic network.md).
...
When you restart an Elasticsearch cluster or node, the system displays the required time. The time is estimated based on your cluster specifications, data structure, and data volume. Typically, it requires a few hours to restart a cluster. For more information, see [Restart a cluster or node](/intl.en-US/Elasticsearch Instances Management/Manage clusters/Restart a cluster or node.md).
...
No, you can scale in your cluster. For more information, see [Scale in an Elasticsearch cluster](/intl.en-US/Elasticsearch Instances Management/Upgrade or downgrade a cluster/Scale in an Elasticsearch cluster.md).
...
We recommend that you add nodes to the cluster when the temporary business surge occurs and remove the nodes after the business surge. For more information, see [Upgrade the configuration of a cluster](/intl.en-US/Elasticsearch Instances Management/Upgrade or downgrade a cluster/Upgrade the configuration of a cluster.md) and [Scale in an Elasticsearch cluster](/intl.en-US/Elasticsearch Instances Management/Upgrade or downgrade a cluster/Scale in an Elasticsearch cluster.md). For the changes to take effect, the system restarts the cluster. Before the restart, take note of the following items:
...
**Note:** In most cases, you do not need to change the settings of the discovery.zen.ping\_timeout, discovery.zen.fd.ping\_timeout, discovery.zen.fd.ping\_interval, and discovery.zen.fd.ping\_retries parameters.
...
The error message returned because of an Elasticsearch V6.7 bug. You must restart your cluster. For more information, see [Restart a cluster or node](/intl.en-US/Elasticsearch Instances Management/Manage clusters/Restart a cluster or node.md). We will fix the bug as soon as possible.
...
You can call the ListSearchLog operation to obtain all logs that you require. For more information, see [ListSearchLog](/intl.en-US/API Reference/Elasticsearch instances/Query logs/ListSearchLog.md).
...
By default, Elasticsearch logs only read and write operations that require 5 to 10 seconds to complete as slow logs. You can log on to the [Kibana console](/intl.en-US/Elasticsearch Instances Management/Data visualization/Kibana/Log on to the Kibana console.md) of the cluster and run the related command to reduce the timestamp precision of log entries. This helps capture more logs. For more information, see [Configure slow logs](/intl.en-US/Elasticsearch Instances Management/Query logs.md).
...
You can call the ListSearchLog operation on a regular basis to obtain the slow logs of your cluster. For more information, see [ListSearchLog](/intl.en-US/API Reference/Elasticsearch instances/Query logs/ListSearchLog.md).
...
-   max\_snapshot\_bytes\_per\_sec: specifies the snapshot creation rate of each node. Default value: 40mb.

## Can I export data from an Elasticsearch cluster to my on-premises machine?

Yes, Elasticsearch provides the snapshot feature. For more information, see [View the snapshot feature](/intl.en-US/Elasticsearch Instances Management/Data backup/View the snapshot feature.md). You can create and store snapshots in OSS and download objects from OSS. For more information, see [Download objects](/intl.en-US/Quick Start/OSS console/Download objects.md).
