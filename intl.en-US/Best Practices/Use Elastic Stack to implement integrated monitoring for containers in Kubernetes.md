...
Elastic Stack provides the integrated monitoring feature. This feature allows you to use Kibana to analyze and display the log data, metrics, and application performance monitoring \(APM\) data of a Container Service for Kubernetes \(ACK\) cluster in a centralized manner. If you deploy your applications in the pods of an ACK cluster, you can view the logs generated by the pods, event metrics of the hosts and network, and APM data in the Kibana console. This facilitates troubleshooting. This topic describes how to implement integrated monitoring for an ACK cluster.
...
-   An ACK cluster is created, and pods are created in the cluster. In this topic, the ACK cluster version 1.18.8-aliyun.1 is used. The specifications of the Elastic Compute Service \(ECS\) instance used for the cluster are 2 vCPUs and 8 GiB of memory.
...
-   kubectl is configured, and the ACK cluster can be accessed by using kubectl.
...
You can use one of the following methods to deploy Metricbeat to the ACK cluster:

-   DaemonSet: Make sure that each node runs one pod. This enables Metricbeat to collect host metrics, system metrics, Docker statistics, and the metrics of all the services that are run on the ACK cluster.
-   Deployment: Deploy a single Metricbeat shipper. The shipper is used to retrieve unique metrics of the ACK cluster, such as metrics for Kubernetes events and kube-state-metrics.

**Note:**

-   In this section, Metricbeat is deployed to an ACK cluster by using both the DaemonSet and Deployment methods. You can also choose to deploy Metricbeat by using only the DaemonSet method.
...
1.  Use kubectl to access the ACK cluster and download the Metricbeat configuration file.
...
2.  Modify the Metricbeat configuration file.
    ...
    **Note:**
    ...
    The official Kubernetes YML configuration file uses the extensions/v1beta1 API version for DaemonSets and Deployments. This API version is deprecated in ACK V1.18 and later. Modify the Metricbeat configuration file to use the apps/v1 API version for Kubernetes, DaemonSets, Deployments, and ReplicaSets in ACK V1.18 and later.
    ...
    1.  Modify the configurations in kind: Deployment and kind: DaemonSet.
        ...
        -   Modify environment variables. The following code provides an example:
            ...
            **Note:** By default, Kibana variables are not defined in the downloaded Metricbeat configuration file. You can use the env parameter to pass the variables.
            ...
            |---------|-----------|
            |ELASTICSEARCH\_HOST|The internal endpoint of the Elasticsearch cluster.|
            |ELASTICSEARCH\_PORT|The private port of the Elasticsearch cluster.|
            |ELASTICSEARCH\_USERNAME|The username of the Elasticsearch cluster. The default value of this parameter is elastic.|
            |ELASTICSEARCH\_PASSWORD|The password of the elastic username.|
    ...
    3.  Change the value of the name parameter. Define the system metrics monitored by the System module and the metrics that can be obtained by the Kubernetes module. The System module monitors the following metrics: CPU, load, memory, and network.
    ...
    4.  Change the value of the name parameter to obtain the metrics for kube-state-metrics and Kubernetes events.

        **Note:** The Metricbeat service is deployed in the kube-system namespace. The kube-state-metrics service is deployed in the arms-prom namespace by default. They belong to different namespaces. Therefore, specify the hosts parameter in the kube-state-metrics.<namespace\>:8080 format. If the Metricbeat and kube-state-metrics services are deployed in the same namespace, specify the hosts parameter in the kube-state-metrics:8080 format.
...
3.  Create resource objects and view the status of the resources.

    Use kubectl to run the following commands:
    ...
    **Note:** You must make sure that the resources in the pods are running. Otherwise, the data may not be displayed in the Kibana console.
...
In this section, Filebeat is deployed by using a DaemonSet controller. This ensures that each node of the ACK cluster runs a pod to collect data. The resources in the configuration file of the shipper are deployed in the kube-system namespace. If you want to change the namespace in which the resources are deployed, you can modify the configuration file.

1.  Download the configuration file of the Filebeat shipper.

    Use kubectl to access the ACK cluster and download the Filebeat configuration file.
...
2.  Modify the Filebeat configuration file.
    ...
    1.  Modify the environment variables in kind: DaemonSet.
    ...
    3.  Configure the ACK cluster to collect logs from the containers in the cluster.
...
1.  Deploy APM Server to a container.
    ...
    1.  Configure the apm-server-yml file.
        ...
        **Note:**
        ...
        -   When you use the Deployment method to deploy APM Server, the docker.elastic.co/apm/apm-server:6.8.14 image is used to deploy the pods. The version of the image must be consistent with that of the Elasticsearch cluster.
        -   Port number 8200 is exposed to the ACK cluster by using the Service object. This ensures that APM agents can communicate with APM Server.
        |Parameter|Description|
        |---------|-----------|
        |ELASTICSEARCH\_HOST|The internal endpoint of the Elasticsearch cluster.|
        |ELASTICSEARCH\_PORT|The private port of the Elasticsearch cluster.|
...
2.  Configure APM agents.
    ...
    1.  Log on to a node of the ACK cluster and create a Dockerfile named myapply in the working directory.
        ...
        The parameters and Java commands that are used for starting the pods are defined in ENTRYPOINT. The following table describes these parameters.
    ...
    5.  Use kubectl to run the following command to deploy the pods:
    ...
    6.  Use a curl command to access the host over port 30000 after all resources in the pods are running.
        ...
        **Note:** 10.7.XX.XX specifies the IP addresses of the nodes in the ACK cluster.
...
-   Problem description: The resources.requests parameter is set to a large value in the configuration file of the ACK cluster. As a result, the pods fail to be started.
...
-   Problem description: Errors keep occurring when I deploy Metricbeat, Filebeat, and Elastic APM. The error message is similar to `no matches for kind "DaemonSet" in version "extensions/v1beat1"`.
    ...
    Solution: Use the apps/v1 API version. This is because that the official Kubernetes YML configuration file uses the extensions/v1beta1 API version for DaemonSets and Deployments. However, in ACK V1.18 and later, the extensions/v1beta1 API version will be deprecated for DaemonSets, Deployments, and ReplicaSets.
