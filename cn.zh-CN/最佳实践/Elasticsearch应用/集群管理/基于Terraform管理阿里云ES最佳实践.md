...
4.  创建RAM用户，并为其授权。
    ...
    2.  创建名为Terraform的RAM用户，并为该用户创建AccessKey。
        ...
        具体操作方法请参见[创建RAM用户](/cn.zh-CN/用户管理/创建RAM用户.md)。

        **说明：** 请不要使用主账号的AccessKey配置Terraform工具。

    3.  为RAM用户授权。

        本示例为用户Terraform授予`AliyunElasticsearchFullAccess`和`AliyunVPCFullAccess`权限，具体操作方法请参见[为RAM用户授权](/cn.zh-CN/用户管理/为RAM用户授权.md)。
