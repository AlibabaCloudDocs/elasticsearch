...
-   创建阿里云Elasticsearch实例，并开启实例的自动创建索引功能。
    ...
    具体操作步骤请参见[步骤一：创建实例](/cn.zh-CN/Elasticsearch/快速开始.md)和[配置YML参数](/cn.zh-CN/Elasticsearch/集群配置/配置YML参数.md)。本文使用6.7版本的实例。
...
-   创建云数据库PolarDB MySQL集群，并开启Binlog。
    ...
    具体操作步骤请参见[购买按量付费集群](/cn.zh-CN/产品价格与购买/购买步骤/购买按量付费集群.md)、[开启Binlog](/cn.zh-CN/内核功能/其它/开启Binlog.md)。

-   创建PolarDB MySQL数据库和表，并插入测试数据。

    具体操作步骤请参见[管理数据库](/cn.zh-CN/用户指南/账号和数据库/管理数据库.md)。本文使用的表结构和测试数据如下。
