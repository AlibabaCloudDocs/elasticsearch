...
-   已经开启了阿里云Elasticsearch实例的X-Pack Watcher功能（默认关闭）。
    ...
    详情请参见[修改YML参数配置](/cn.zh-CN/ES实例/集群配置/配置YML文件/修改YML参数配置.md)。

-   已经购买了阿里云ECS实例。

    购买的ECS实例要与阿里云Elasticsearch实例在同一个区域和专有网络VPC（Virtual Private Cloud）下，并且能够访问公网，详情请参见[步骤一：创建ECS实例](/cn.zh-CN/快速入门/通过控制台使用ECS实例（详细版）/Linux系统实例快速入门.md)。
...
1.  配置ECS安全组。
    ...
    5.  填写相关参数。
        ...
        ![添加安全组规则](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/6706320061/p49922.png)
        ...
        |**授权对象**|添加您购买的阿里云Elasticsearch实例所有节点的IP地址。 **说明：** 获取Elasticsearch实例所有节点的IP地址列表：参见[登录Kibana控制台](/cn.zh-CN/ES实例/可视化控制/Kibana/登录Kibana控制台.md)，登录对应实例的Kibana控制台，单击左侧导航栏的**Monitoring**，再单击**Nodes**。 |
        |**描述**|输入对规则的描述。|
...
2.  配置Nginx代理。
    ...
    2.  配置nginx.conf文件。
        ...
        ![ngnix.conf的server部分配置](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/1463533061/p175712.png)
...
3.  设置报警。
    ...
    3.  在**Console**中执行如下命令创建一个报警文档。
        ...
        **说明：**
        ...
        -   如果在执行以上命令时出现`No handler found for uri [/_xpack/watcher/watch/log_error_watch_2] and method [PUT]`异常，表示您购买的阿里云Elasticsearch实例未开启X-Pack Watcher功能，请开启后再执行以上命令，详情请参见[修改YML参数配置](/cn.zh-CN/ES实例/集群配置/配置YML文件/修改YML参数配置.md)。
