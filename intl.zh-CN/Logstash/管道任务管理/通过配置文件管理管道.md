...
-   创建阿里云Elasticsearch实例，并开启自动创建索引功能。
    ...
    创建实例的具体操作，请参见[创建阿里云Elasticsearch实例](/intl.zh-CN/Elasticsearch/快速入门/步骤一：创建实例/创建阿里云Elasticsearch实例.md)；开启自动创建索引功能的具体操作，请参见[开启自动创建索引](/intl.zh-CN/Elasticsearch/快速入门/步骤二：配置实例（可选）.md)。
